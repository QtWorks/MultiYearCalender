  ...
  QList<QTableWidgetItem *> selezioni = selectedItems();
	...
	if (selezioni.size() < 0) {
		...
		return;
		     ...
		     notatxt = QInputDialog::getText(this, tr("La vostra nota (Mass. 110 char.)"),tr("Nota:"), QLineEdit::Normal,pday.note);
         if (notatxt.size() < 1) {
					 return;
				 }
...
void Calendar::contextMenuEvent ( QContextMenuEvent * e )
{
      ...
      QMenu *Tanni = new QMenu(tr("Open %1").arg(ActiveYear));
    ...
    /* Reservation modus 0libero / 1 = booked / 2 = booked provisional / 3 = reserved / 10 = closed / */
         QAction *open0 = new QAction(tr("make free"),this);
    open0->setStatusTip(tr("make free"));
	  open0->setData(0);
    open0->setIcon ( aday.createColorToolButtonIcon(0) );
    connect(open0, SIGNAL(triggered()), this,  SLOT(Riservati()));
	
        QAction *open1 = new QAction(tr("reserved"),this);
    open1->setStatusTip(tr("reserved"));
	  open1->setData(1);
	  open1->setIcon ( aday.createColorToolButtonIcon(1) );
    connect(open1, SIGNAL(triggered()), this,  SLOT(Riservati()));
	
      QAction *open2 = new QAction(tr("provisionally Confidential"),this);
    open2->setStatusTip(tr("provisionally Confidential"));
	  open2->setData(2);
	  open2->setIcon ( aday.createColorToolButtonIcon(2) );
    connect(open2, SIGNAL(triggered()), this,  SLOT(Riservati()));
	
        QAction *open3 = new QAction(tr("Busy period"),this);
    open3->setStatusTip(tr("Busy period"));
	  open3->setData(3);
		open3->setIcon ( aday.createColorToolButtonIcon(3) );
    connect(open3, SIGNAL(triggered()), this,  SLOT(Riservati()));
		
        QAction *open10 = new QAction(tr("period Closed"),this);
    open10->setStatusTip(tr("period Closed"));
	  open10->setData(10);
    ...
    TContext->addAction(tr( "Export to file" ), this , SLOT( ExportAsFile() ) );
        TContext->addAction(tr( "Import from File" ), this , SLOT( ImportFromFile() ) );
